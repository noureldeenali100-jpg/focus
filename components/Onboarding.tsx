...
const GuardianLogo = () => (
  <svg width="100" height="100" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M60 10L105 28V60C105 85 86 108 60 115C34 108 15 85 15 60V28L60 10Z" fill="url(#premium-shield-grad)" />
    <path d="M60 18L97 33V60C97 80 82 99 60 106C38 99 23 80 23 60V33L60 18Z" fill="white" fillOpacity="0.15" />
...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const nextStep = () => {
    ...
    else onComplete(name, canvasRef.current?.toDataURL('image/png', 1.0) || '');
  };
        ...
        >
          {step === 'WELCOME' && (
            <div className="w-full text-center">
              <div className="mb-10 flex justify-center"><GuardianLogo /></div>
              ...
              />
              <button disabled={!name.trim()} onClick={nextStep} className="w-full py-5 rounded-full font-black text-lg bg-slate-900 dark:bg-white text-white dark:text-slate-900 disabled:opacity-20 active:scale-95 transition-all">Initialize</button>
            </div>
          )}
          ...
          {step === 'COMMITMENT' && (
            <div className="w-full text-center">
              ...
              <div className="bg-slate-50 dark:bg-slate-900/50 rounded-[40px] p-10 mb-10 border border-slate-100 dark:border-slate-800">
                <p className="italic text-xl font-bold text-slate-700 dark:text-slate-300">"I hereby commit to reclaiming my focus and dedicating this space to meaningful work."</p>
              </div>
              <button onClick={() => setAgreed(!agreed)} className={`w-full flex items-center justify-between p-5 rounded-full border mb-8 transition-all ${agreed ? 'bg-[var(--accent-color)] text-white' : 'bg-white dark:bg-slate-900 border-slate-200 text-slate-400'}`}>
                <span className="font-black uppercase text-xs pl-4">{agreed ? 'Agreed' : 'Accept Protocol'}</span>
                <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center">{agreed && <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><polyline points="20 6 9 17 4 12"/></svg>}</div>
              </button>
              <button disabled={!agreed} onClick={nextStep} className="w-full py-5 rounded-full font-black text-lg bg-slate-900 dark:bg-white text-white dark:text-slate-900 disabled:opacity-20 active:scale-95 transition-all">Continue</button>
            </div>
          )}
          ...
          {step === 'SIGNATURE' && (
            <div className="w-full text-center">
              ...
              <div className="w-full h-64 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-[32px] overflow-hidden mb-8 relative touch-none shadow-inner">
                <canvas 
                  ref={canvasRef} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={stopDrawing} onPointerLeave={stopDrawing}
              ...
              <div className="flex gap-4">
                ...
                <button disabled={!hasSigned} onClick={nextStep} className="flex-[2] py-4 rounded-full text-[10px] font-black uppercase tracking-widest bg-[var(--accent-color)] text-white shadow-xl shadow-[var(--accent-color)]/20 disabled:opacity-20 transition-all active:scale-95">Verify Seal</button>
              </div>
            </div>
          )}
          ...
          {step.startsWith('TUTORIAL') && (
            <div className="w-full text-center">
              ...
              <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter mb-6">{step === 'TUTORIAL_BLOCKING' ? 'Focus Shield' : step === 'TUTORIAL_TIMER' ? 'Peak Flow' : 'Ready'}</h2>
              <p className="text-slate-500 text-lg mb-12">{step === 'TUTORIAL_BLOCKING' ? 'Social media is blocked. Deep work is your default.' : step === 'TUTORIAL_TIMER' ? 'Define a goal, set a timer, and commit to the session.' : `Initialization complete, ${name}. Your sanctuary awaits.`}</p>
              <button onClick={nextStep} className="w-full py-5 rounded-full font-black text-lg bg-slate-900 dark:bg-white text-white dark:text-slate-900 active:scale-95 transition-all">{step === 'TUTORIAL_SUMMARY' ? 'Enter Sanctuary' : 'Next'}</button>
            </div>
          )}
        </motion.div>
      </AnimatePresence>
