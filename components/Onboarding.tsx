...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const renderWelcome = () => (
    <div className="flex-1 flex flex-col items-center justify-center px-10 text-center animate-in fade-in duration-500 bg-white dark:bg-slate-950 w-full">
      <div className="max-w-md w-full flex flex-col items-center">
        <div className="w-24 h-24 md:w-32 md:h-32 rounded-[2.2rem] md:rounded-[3rem] flex items-center justify-center rotate-12 mb-10 shadow-2xl shadow-[var(--accent-color)]/30 dark:shadow-none transition-all duration-700 hover:rotate-0 hover:scale-105" style={{ backgroundColor: 'var(--accent-color)', color: 'white' }}>
           <GuardianLogo />
        </div>
        ...
        <input 
          ...
          className="w-full bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-3xl px-8 py-5 text-center text-xl font-bold text-slate-800 dark:text-slate-100 outline-none transition-all mb-8 placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:border-[var(--accent-color)] shadow-sm"
        />
        <button 
          disabled={!name.trim()}
          onClick={nextStep}
          style={{ backgroundColor: name.trim() ? 'var(--accent-color)' : 'transparent' }}
          className={`w-full py-6 rounded-3xl font-black text-xl shadow-xl transition-all ${name.trim() ? 'text-white hover:opacity-90 active:scale-95' : 'bg-slate-100 dark:bg-slate-800 text-slate-300 dark:text-slate-600 cursor-not-allowed'}`}
        >
          Begin Journey
        </button>
      </div>
  ...
  const renderCommitment = () => (
    <div className="flex-1 flex flex-col items-center justify-center px-10 text-center animate-in slide-in-from-right duration-500 bg-white dark:bg-slate-950 w-full">
      <div className="max-w-md w-full">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 rounded-[40px] p-10 border border-slate-100 dark:border-slate-800 mb-10 text-slate-600 dark:text-slate-300 shadow-sm">
          <div className="w-14 h-14 mx-auto mb-6 text-[var(--accent-color)]">
             <GuardianLogo />
          </div>
          <p className="leading-relaxed italic text-lg md:text-xl mb-8 font-medium">
            “I promise to stay focused and finish my tasks.”
          </p>
          <label className="flex items-center space-x-4 cursor-pointer group bg-white dark:bg-slate-950 p-5 rounded-2xl border border-slate-100 dark:border-slate-800">
            <input 
              type="checkbox" 
              checked={agreed}
              onChange={(e) => setAgreed(e.target.checked)}
              className="w-7 h-7 rounded-lg border-slate-300 dark:border-slate-700 focus:ring-[var(--accent-color)] bg-white dark:bg-slate-950"
        ...
        <button 
          ...
          className={`w-full py-6 rounded-3xl font-black text-xl shadow-xl transition-all ${agreed ? 'text-white hover:opacity-90 active:scale-95' : 'bg-slate-100 dark:bg-slate-800 text-slate-300 dark:text-slate-600 cursor-not-allowed'}`}
        >
          Confirm Commitment
        </button>
      </div>
  ...
  const renderSignature = () => (
    <div className="flex-1 flex flex-col items-center justify-center px-10 text-center animate-in slide-in-from-right duration-500 bg-white dark:bg-slate-950 w-full">
      <div className="max-w-md w-full">
        ...
        <div className="w-full relative bg-slate-50 dark:bg-slate-950 border-2 border-slate-100 dark:border-slate-800 rounded-[40px] overflow-hidden mb-10 h-64 touch-none shadow-inner">
          <canvas 
            ref={canvasRef}
            onMouseDown={onStart}
            onMouseMove={onMove}
        ...
        <div className="flex gap-4 w-full">
          <button 
            ...
            className="flex-1 py-5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-3xl text-xs font-black uppercase tracking-[0.2em] active:scale-95 transition-transform"
          >
            Clear
          </button>
          <button 
            ...
            className={`flex-1 py-5 rounded-3xl text-xs font-black uppercase tracking-[0.2em] transition-all ${hasSigned ? 'text-white shadow-xl active:scale-95' : 'bg-slate-100 dark:bg-slate-800 text-slate-300 dark:text-slate-600 cursor-not-allowed'}`}
          >
            Save Pledge
          </button>
        </div>
  ...
  const renderTutorialBlocking = () => (
    <div className="flex-1 flex flex-col items-center justify-center px-10 text-center animate-in slide-in-from-right duration-500 bg-white dark:bg-slate-950 w-full">
      <div className="max-w-md w-full">
        <div className="w-20 h-20 bg-red-50 dark:bg-red-900/20 text-red-500 rounded-3xl flex items-center justify-center mb-8 mx-auto shadow-sm">
        ...
        <button 
          ...
          className="w-full py-6 rounded-3xl font-black text-xl text-white shadow-xl hover:opacity-90 active:scale-95 transition-all"
        >
  ...
  const renderTutorialTimer = () => (
    <div className="flex-1 flex flex-col items-center justify-center px-10 text-center animate-in slide-in-from-right duration-500 bg-white dark:bg-slate-950 w-full">
      <div className="max-w-md w-full">
        <div className="w-20 h-20 bg-blue-50 dark:bg-blue-900/20 text-blue-500 rounded-3xl flex items-center justify-center mb-8 mx-auto shadow-sm">
  ...
  const renderTutorialSummary = () => (
    <div className="flex-1 flex flex-col items-center justify-center px-10 text-center animate-in slide-in-from-right duration-500 bg-white dark:bg-slate-950 w-full">
      <div className="max-w-md w-full">
        <div className="w-24 h-24 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-500 rounded-[2.5rem] flex items-center justify-center mb-10 mx-auto shadow-inner">
