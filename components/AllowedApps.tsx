...
const AllowedApps: React.FC<AllowedAppsProps> = ({ 
  ...
  return (
    <div className="p-8 pb-32 bg-slate-50 dark:bg-slate-950 min-h-full overflow-y-auto no-scrollbar">
      <header className="mb-12 relative animate-in fade-in slide-in-from-top-4 duration-700">
      ...
      <div className="space-y-6 mb-14 animate-in fade-in slide-in-from-bottom-4 duration-700 stagger-2">
        ...
        {CYCLE_APPS_BASE.map((app, i) => {
          ...
          return (
            <div 
              key={app.id} 
              className={`bg-white dark:bg-slate-900 border transition-all duration-500 cubic-bezier(0.16, 1, 0.3, 1) rounded-[36px] overflow-hidden border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md hover:-translate-y-0.5 animate-in fade-in slide-in-from-right-4`}
              style={{ animationDelay: `${(i+2) * 100}ms` }}
            >
              <div className="p-6 flex items-center justify-between">
                <div className="flex items-center space-x-5">
                  <div className="w-16 h-16 bg-transparent flex items-center justify-center shrink-0 transition-transform duration-500 group-hover:scale-105">
      ...
      <div className="space-y-4 mb-16 animate-in fade-in slide-in-from-bottom-4 duration-700 stagger-3">
        ...
        >
          <span className="text-xs font-black uppercase tracking-[0.1em] text-slate-700 dark:text-slate-200">Add to Block List</span>
        ...
        <div className="grid grid-cols-1 gap-4">
          {allRestricted.map((app, i) => {
            ...
            return (
              <div 
                key={app.id} 
                className={`flex items-center justify-between p-5 bg-white dark:bg-slate-900 rounded-[32px] border transition-all duration-500 cubic-bezier(0.16, 1, 0.3, 1) animate-in fade-in slide-in-from-right-4 ${isUnlocked ? 'border-emerald-500/40 ring-4 ring-emerald-500/5 scale-[1.02]' : 'border-slate-100 dark:border-slate-800'}`}
                style={{ animationDelay: `${(i+5) * 80}ms` }}
              >
                <div className="flex items-center space-x-4">
                  <div className={`w-14 h-14 bg-transparent flex items-center justify-center transition-all duration-700 ${!isUnlocked ? 'grayscale opacity-60 scale-95' : 'grayscale-0 opacity-100 scale-100'}`}>
                    <AppIcon appId={app.id} className="w-full h-full" />
                  </div>
                  <div>
                    <p className="font-bold text-slate-700 dark:text-slate-200 leading-tight">{app.name}</p>
                    {isPermanent ? (
                      <p className="text-[10px] font-black text-red-500 uppercase mt-1 tracking-tighter opacity-80">Hard Blocked</p>
                    ) : isPending ? (
                      ...
                      <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase mt-1">Wait 1h to use</p>
                    )}
                  </div>
                </div>
                ...
                {!isUnlocked && !isPermanent && (
                  <button 
                    ...
                    className={`px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 active:scale-75 ${isPending ? 'bg-slate-100 text-slate-400 dark:bg-slate-800' : 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 shadow-xl hover:brightness-110'}`}
                  >
                ...
                {isPermanent && (
                  <div className="p-2.5 text-slate-200 dark:text-slate-700 animate-in zoom-in duration-500">
      ...
      {isPickerOpen && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center animate-in fade-in duration-300 backdrop-blur-sm">
          <div className="absolute inset-0 bg-slate-950/40" onClick={() => setIsPickerOpen(false)} />
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-t-[48px] shadow-[0_-20px_50px_-12px_rgba(0,0,0,0.3)] p-10 pb-14 animate-in slide-in-from-bottom-full duration-500 cubic-bezier(0.16, 1, 0.3, 1) max-h-[90vh] overflow-hidden flex flex-col">
            <div className="w-16 h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full mx-auto mb-8 shrink-0 animate-in slide-in-from-top-4 duration-700" />
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar space-y-4">
              {availableToBlock.map((app, i) => (
                <button 
                  ...
                  className="w-full flex items-center space-x-5 p-5 rounded-[28px] border-2 border-slate-50 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all active:scale-[0.96] animate-in fade-in slide-in-from-right-4"
              ...
              {availableToBlock.length === 0 && (
                <div className="text-center py-16 opacity-40 animate-in zoom-in duration-1000">
                  <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-500">Full Protection Active</p>
                </div>
              )}
            </div>
            ...
            <button 
              ...
              className="mt-8 w-full py-5 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-3xl text-xs font-black uppercase tracking-widest active:scale-95 transition-all duration-300"
            >
              Close
            </button>
          </div>
