...
const Tasks: React.FC<TasksProps> = ({ 
  ...
  const handleSaveEdit = () => {
    if (editingId && editingText.trim()) {
      onUpdateTask(editingId, sanitize(editingText, 100), sanitize(editingDesc, 500));
      setEditingId(null);
    }
  };
          ...
          {isAdding && (
            ...
            >
              <div className="space-y-1">
                ...
                <input 
                  type="text" value={newTaskText} onChange={(e)=>setNewTaskText(e.target.value.slice(0, 100))} placeholder="e.g., Deep Work Session"
              ...
              <div className="space-y-1">
                ...
                <textarea 
                  value={newTaskDesc} onChange={(e)=>setNewTaskDesc(e.target.value.slice(0, 500))} placeholder="What are the key steps for this objective?"
              ...
              <button 
                ...
                onClick={() => { onAddTask(sanitize(newTaskText, 100), sanitize(newTaskDesc, 500)); setNewTaskText(''); setNewTaskDesc(''); setIsAdding(false); }}
        ...
        <motion.div layout className="space-y-5 pb-40 px-2">
          <AnimatePresence mode="popLayout">
            {sortedTasks.length === 0 && !isAdding && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.5 }} className="py-20 flex flex-col items-center justify-center text-center space-y-4">
                <div className="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 5v14M5 12h14"/></svg></div>
            ...
            {sortedTasks.map((task) => (
              <motion.div 
                key={task.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={transition}
              ...
              >
                <div className="flex items-start p-6">
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={()=>onToggleTask(task.id)} className={`mt-1 shrink-0 w-8 h-8 rounded-2xl border-2 flex items-center justify-center transition-all ${task.completed ? 'bg-emerald-500 border-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800'}`}>
                    ...
                    {editingId === task.id ? (
                      <div className="flex flex-col gap-4 animate-in fade-in duration-300">
                        <div className="space-y-1">
                          ...
                          <input autoFocus type="text" value={editingText} onChange={(e) => setEditingText(e.target.value.slice(0, 100))} className="w-full bg-transparent text-base font-black outline-none border-b-2 border-slate-100 dark:border-slate-700 focus:border-[var(--accent-color)] transition-all py-1" />
                        </div>
                        <div className="space-y-1">
                          <label className="text-[8px] font-black uppercase text-[var(--accent-color)] tracking-widest ml-1">Context</label>
                          <textarea value={editingDesc} onChange={(e) => setEditingDesc(e.target.value.slice(0, 500))} className="w-full bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-4 text-xs font-medium outline-none border border-slate-100 dark:border-slate-700 focus:border-[var(--accent-color)] transition-all resize-none h-24" placeholder="Add steps or context..." />
                        </div>
                        <div className="flex items-center gap-3 pt-2">
                           <button onClick={handleSaveEdit} className="flex-1 py-3 bg-[var(--accent-color)] text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-[var(--accent-color)]/20 active:scale-95 transition-all">Save Changes</button>
                           <button onClick={() => setEditingId(null)} className="px-6 py-3 bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 rounded-2xl text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all">Cancel</button>
                        </div>
                      </div>
                    ) : (
                      <div className="cursor-pointer" onClick={() => setExpandedId(expandedId === task.id ? null : task.id)}>
                        <button onClick={(e) => { e.stopPropagation(); if(!isTimerActive) onSetActiveTask(task.id === activeTaskId ? null : task.id); }} className={`text-left text-base font-black tracking-tight leading-tight transition-all duration-300 ${task.completed ? 'text-slate-300 dark:text-slate-600 line-through' : activeTaskId === task.id ? 'text-[var(--accent-color)]' : 'text-slate-800 dark:text-slate-100'}`}>
                          {task.text}
                        </button>
                        <AnimatePresence>
                          {(task.description || (expandedId === task.id)) && !task.completed && (
                            <motion.p layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`text-[11px] font-medium leading-relaxed mt-1.5 transition-colors ${expandedId === task.id ? 'text-slate-500 dark:text-slate-400' : 'text-slate-400 dark:text-slate-500 truncate'}`}>
                  ...
                  {!editingId && (
                    <div className="flex flex-col space-y-1 opacity-0 group-hover:opacity-100 transition-all duration-300 -mr-2">
                      {!task.completed && (
                        <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleStartEdit(task)} className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-[var(--accent-color)] hover:bg-[var(--accent-subtle)] rounded-full transition-all"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></motion.button>
                      )}
                      <motion.button whileTap={{ scale: 0.9 }} onClick={()=> { if(confirm('Permanently delete this objective?')) onDeleteTask(task.id); }} className="w-10 h-10 flex items-center justify-center text-slate-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full transition-all"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></motion.button>
                    </div>
                  )}
                </div>
              </motion.div>
