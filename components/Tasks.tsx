    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950">
      ...
      <section className="flex-1 bg-white dark:bg-slate-900 rounded-t-[40px] shadow-2xl px-8 pt-10 overflow-y-auto no-scrollbar z-20 animate-in slide-in-from-bottom-10 duration-700">
        <div className="flex items-center justify-between mb-8 animate-in fade-in stagger-2">
          ...
          <span className="text-[10px] font-black text-[var(--accent-color)] uppercase tracking-widest bg-[var(--accent-subtle)] px-3 py-1.5 rounded-xl transition-all duration-500 hover:scale-105">
        ...
        <div className="flex space-x-3 mb-10 group animate-in fade-in stagger-3">
          ...
          <button 
            onClick={handleAdd}
            className="bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 w-14 h-14 rounded-2xl flex items-center justify-center active:scale-75 shadow-xl transition-all duration-300 hover:brightness-110 hover:-translate-y-0.5"
          >
            ...
            >
              <div className="flex items-center space-x-4 flex-1 overflow-hidden">
                <button 
                  ...
                  className={`shrink-0 w-7 h-7 rounded-xl border-2 flex items-center justify-center transition-all duration-300 active:scale-50 ${task.completed ? 'bg-emerald-500 border-emerald-500 text-white shadow-lg shadow-emerald-500/30' : 'border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 hover:border-[var(--accent-color)]'}`}
                >
                ...
                <button 
                  ...
                  className={`flex-1 text-left text-base font-bold truncate transition-all duration-500 ${task.completed ? 'text-slate-300 dark:text-slate-600 line-through' : activeTaskId === task.id ? 'text-[var(--accent-color)]' : 'text-slate-700 dark:text-slate-200'} active:opacity-60`}
