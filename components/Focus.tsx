          ...
          {showSounds && (
            <div className="absolute top-full mt-3 left-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl rounded-3xl p-2.5 flex flex-col min-w-[140px] z-50 animate-zoom-in">
              {soundOptions.map((opt) => (
                <button key={opt.id} onClick={() => {onSetFocusSound(opt.id); setShowSounds(false);}} className={`py-3 px-4 text-[10px] font-black uppercase tracking-widest rounded-xl text-start transition-all duration-300 active:scale-95 ${focusSound === opt.id ? 'bg-[var(--accent-subtle)] text-[var(--accent-color)]' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                  {opt.label}
                </button>
              ))}
            </div>
