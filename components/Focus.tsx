...
const Focus: React.FC<FocusProps> = ({ 
  ...
  const enterFullscreen = useCallback(async () => {
    ...
    try {
      ...
      // Fallback for devices where FS API is blocked or restricted
      setIsAppFullscreen(true); 
    }
  }, [setIsAppFullscreen]);
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 transition-colors duration-400 ease-out relative">
      <section className="flex-1 flex flex-col items-center justify-center px-6 py-[4vh] relative overflow-hidden">
        <div className="absolute top-5 left-0 right-0 flex justify-between px-5 z-50">
          <button onClick={() => setShowSounds(!showSounds)} className={`p-2 rounded-2xl bg-white/20 dark:bg-slate-900/40 border border-slate-200/30 dark:border-slate-700 backdrop-blur-sm active:scale-90 transition-all duration-300 shadow-sm ${focusSound !== 'none' ? 'text-[var(--accent-color)] border-[var(--accent-color)]/20' : 'text-slate-400 dark:text-slate-400'}`}>
          ...
          <button onClick={enterFullscreen} className="p-2 rounded-2xl bg-white/20 dark:bg-slate-900/40 border border-slate-200/30 dark:border-slate-700 backdrop-blur-sm active:scale-90 transition-all duration-300 shadow-sm text-slate-400 dark:text-slate-400">
          ...
          {showSounds && (
            <div className="absolute top-full mt-3 left-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl rounded-3xl p-2.5 flex flex-col min-w-[140px] z-50 animate-zoom-in">
              {[
                { id: 'none', label: 'Mute' }, 
                { id: 'rain', label: 'Rain' }, 
                { id: 'clock', label: 'Ticking' }, 
                { id: 'library', label: 'Library' }
              ].map((opt) => (
                <button key={opt.id} onClick={() => {onSetFocusSound(opt.id as FocusSound); setShowSounds(false);}} className={`py-3 px-4 text-[10px] font-black uppercase tracking-widest rounded-xl text-start transition-all duration-300 active:scale-95 ${focusSound === opt.id ? 'bg-[var(--accent-subtle)] text-[var(--accent-color)]' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                  {opt.label}
                </button>
              ))}
            </div>
        ...
        <div className="text-center mb-[4vh] z-10 flex flex-col items-center animate-fade-in">
          <h4 className="text-[clamp(9px,1.2vh,11px)] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-1">{isStopwatchMode ? 'Stopwatch' : 'Focus Timer'}</h4>
        ...
        <div className="mb-[5vh] flex items-center justify-center w-full relative">
          <div 
            className="relative flex items-center justify-center transform-gpu transition-all duration-300 ease-out cursor-pointer z-20" 
            style={{ width: size, height: size, willChange: 'transform' }}
            onClick={enterFullscreen}
          >
            <svg className="absolute w-full h-full -rotate-90 transform-gpu" viewBox="0 0 220 220" style={{ transformStyle: 'preserve-3d' }}>
              ...
              <circle cx={110} cy={110} r={radius} stroke="var(--accent-color)" strokeWidth={strokeWidth} fill="transparent" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" className="transition-[stroke-dashoffset] duration-300 ease-linear" />
            </svg>
            <div className="flex flex-col items-center justify-center text-center z-20 pointer-events-none px-4">
              <TimerDigits 
                seconds={timerSeconds} 
                className={`text-[clamp(3rem,8vh,5rem)] font-black tracking-tighter leading-none transform-gpu transition-colors duration-300 ease-out ${isTimerActive ? 'text-slate-900 dark:text-white' : 'text-slate-300 dark:text-slate-500'}`} 
              />
        ...
        <div className="flex flex-row items-center justify-center gap-[4vw] md:gap-8 z-30 relative w-full mb-[5vh]">
          <div className={`relative transition-all duration-300 ease-out transform-gpu ${isTimerActive ? '-translate-x-[200px] opacity-0 pointer-events-none' : 'translate-x-0 opacity-100'}`}>
            <button 
              ...
              className={`w-[clamp(48px,12vw,60px)] h-[clamp(48px,12vw,60px)] flex items-center justify-center rounded-[35%] border transition-all duration-300 ease-out active:scale-90 ${isStopwatchMode ? 'bg-[var(--accent-subtle)] border-[var(--accent-color)]/40 text-[var(--accent-color)]' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-400'} shadow-sm`}
            >
              {isStopwatchMode ? (
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M10 2h4"/><path d="M12 14v-4"/><path d="M4 13a8 8 0 1 0 16 0 8 8 0 1 0-16 0z"/><path d="M12 2v2"/></svg>
              ) : (
                ...
                <div className="mt-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                  <div className="flex items-center space-x-2 px-1">
                    ...
                    <button 
                      ...
                      className="shrink-0 w-9 h-9 bg-[var(--accent-color)] text-white rounded-xl flex items-center justify-center active:scale-90 transition-transform duration-300"
                    >
          ...
          <button onClick={onToggleTimer} className={`w-[clamp(70px,18vw,85px)] h-[clamp(70px,18vw,85px)] rounded-[38%] flex items-center justify-center transition-all duration-300 shadow-2xl active:scale-90 hover:brightness-110 z-30 ${isTimerActive ? 'bg-orange-500 text-white' : 'bg-[var(--accent-color)] text-white'}`}>
          ...
          <div className={`transition-all duration-300 ease-out transform-gpu ${isTimerActive ? 'translate-x-[200px] opacity-0 pointer-events-none' : 'translate-x-0 opacity-100'}`}>
            <button disabled={isTimerActive} onClick={onToggleMode} className={`w-[clamp(48px,12vw,60px)] h-[clamp(48px,12vw,60px)] flex items-center justify-center rounded-[35%] border transition-all duration-300 ease-out active:scale-90 ${isStopwatchMode ? 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-400' : 'bg-[var(--accent-subtle)] border-[var(--accent-color)]/40 text-[var(--accent-color)]'} shadow-sm`}>
              {isStopwatchMode ? (
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
              ) : (
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M10 2h4"/><path d="M12 14v-4"/><path d="M4 13a8 8 0 1 0 16 0 8 8 0 1 0-16 0z"/><path d="M12 2v2"/></svg>
        ...
        <div className="z-10 text-center max-w-[320px] h-[6vh] flex items-center justify-center mb-4 transition-all duration-400 ease-out px-4">
          <p key={quoteIndex} className="text-[clamp(10px,1.4vh,12px)] font-black uppercase tracking-[0.2em] leading-relaxed transition-all duration-400 transform-gpu" style={{ color: isTimerActive ? 'var(--accent-color)' : 'rgb(148, 163, 184)', opacity: isTimerActive ? 1 : 0.6 }}> 
            {isTimerActive ? currentQuote : 'Stay focused'} 
          </p>
        </div>
      </section>
      ...
      {isAppFullscreen && (
        <div 
          ...
          className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center animate-in fade-in duration-300 overflow-hidden cursor-pointer"
        >
          <TimerDigits 
            seconds={timerSeconds} 
